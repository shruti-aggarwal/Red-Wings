...
th, td {
  ...
  padding: 8px;
    ...
    <nav class="navbar navbar-expand-sm bg-dark navbar-dark"">
      ...
      <table>
  <tr><td><b> Name</td><td></td><td><b>Phone</td></tr>
    ...
    if( isset($_POST['search']) ) {
      ...
      $state=$_POST['state'];
      $query=mysqli_query($con,"SELECT * FROM bloodbanks WHERE state like '%$state%'");
      if(mysqli_num_rows($query)>0){
        while($row=mysqli_fetch_array($query)){
          echo "<tr><td>".$row['name']."</td><td></td><td>".$row['phone']."</td></tr>";
  }
}else{
